import { Home, PanelLeft, User2, FileText } from 'lucide-react';
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navigation = [
    ...
    { name: 'Document', to: paths.app.document.getHref(), icon: FileText },
  ].filter(Boolean) as SideNavigationItem[];

  return (
    <div className="flex min-h-screen w-full flex-col bg-white/40">
      <aside className="fixed inset-y-0 left-0 z-10 hidden w-60 flex-col border-r bg-red-700 sm:flex">
      ...
      <div className="flex flex-col sm:gap-4 sm:py-4 sm:pl-60">
        <header className="fixed inset-0 z-30 flex h-20 items-center justify-between gap-4 bg-red-700 px-4">
          <Progress />
          <Drawer>
